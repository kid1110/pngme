...
impl Png {
    ...
    pub fn from_file<P: AsRef<Path>>(path: P,chunk_type:&str,chunk_data:&str)->Result<()>{
        ...
        let chunk = chunk_from_strings(chunk_type, chunk_data)?;
        res.append_chunk(chunk);
        println!("res: {:?}",res.as_bytes().as_slice());
        println!("----------------------");
        file.seek(SeekFrom::Start(0))?;
        let res = file.write_all(res.as_bytes().as_slice());
        match res{
    ...
    pub fn get_chunk<P:AsRef<Path>>(chunk_type: &str,path:P)->Result<Chunk>{
        ...
        return Err(Error::from("could not decode chunk by chunk_type you give"))
    }
    pub fn get_chunks_from_file<P:AsRef<Path>>(path:P)->Result<Vec<Chunk>>{
        let mut file = File::open(path)?;
        let mut buffer = Vec::new();
        file.read_to_end(&mut buffer)?;
        let res = Png::try_from(buffer.as_slice())?;
    ...
    pub fn remove_chunk<P:AsRef<Path>>(chunk_type: &str,path:P) -> Result<Chunk>{
        ...
        println!("----------------------");
        let mut res = Png::try_from(buffer.as_slice())?;
        println!("beforepng: {:?}",res);
        println!("----------------------");
        let chunk_type = ChunkType::from_str(chunk_type).unwrap();
        for(index,item) in res.chunks.iter().enumerate(){
            if item.chunk_type().bytes() == chunk_type.bytes(){
                let chunk = res.chunks.swap_remove(index);
                //write in file
                println!("png: {:?}",res);
                println!("----------------------");
                let mut file = File::create(&path)?;
                file.write_all(res.as_bytes().as_slice())?;
                return Ok(chunk);
            }
        }
        Err(Error::from("chunks don't have this value"))
    }
    fn header(&self) -> &[u8; 8]{
        &self.header
    }
    ...
    fn chunk_by_type(&self, chunk_type: &str) -> Option<&Chunk>{
        ...
        for chunk in &self.chunks{
            if chunk.chunk_type().bytes() == chunk_type.bytes(){
                return Some(&chunk)
            }
        }
        return None
...
pub fn chunk_from_strings(chunk_type: &str, data: &str) -> Result<Chunk> {
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> std::result::Result<Self, Self::Error> {
        ...
        let header:[u8;8] = value[0..8].try_into()?;
        if header != Png::STANDARD_HEADER{
            
            Err(Error::from("invalid png header"))
        }else{
            let mut chunks:Vec<Chunk> = Vec::new();
            let mut index = 11;
            while index <value.len(){
                let mut fixed_array: [u8; 4] = [0; 4];
                fixed_array.copy_from_slice(&value[index-3..index+1]);
                let length = u32::from_be_bytes(fixed_array) as usize;
                let chunk_data = &value[index-3..index+length+9];
                let chunk = Chunk::try_from(chunk_data)?;
                chunks.push(chunk);
                index = index + length+12;
            } 
            let res = Png{
                header,
                chunks
...
mod tests {
    ...
    // #[test]
    // fn test_get_chunk(){
    //     let mut res = Png::from_file(Path::new("./test.png")).unwrap();
    //     let chunk = res.get_chunk("rust");
    //     assert!(chunk.is_ok())
    // }
    // #[test]
    // fn test_from_file(){
    //     let res = Png::from_file(Path::new("./test.png"));
    //     assert!(res.is_ok())
    // }
    ...
    // // 创建文件并将图像保存为 PNG 格式
    // image.save(Path::new("test.png")).expect("Failed to save image");
    // }
    #[test]
    fn test_as_bytes() {
